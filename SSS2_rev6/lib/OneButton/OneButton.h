...
public:
  // ----- Constructor -----
  OneButton(uint8_t pin, bool active);
  
  // ----- Set runtime parameters -----

  // set # millisec after single click is assumed.
  void setClickTicks(uint32_t ticks);

  // set # millisec after press is assumed.
  void setPressTicks(uint32_t ticks);
...
private:
  ...
  uint32_t _clickTicks; // number of ticks that have to pass by before a click is detected
  uint32_t _pressTicks; // number of ticks that have to pass by before a long button press is detected
  const uint32_t _debounceTicks = 50; // number of ticks for debounce times.
